...
function sortByDuration(data, flightLength) {
    if (flightLength <= 60) {
        // console.log(data.map(({title, duration}, index) => ({title: title, duration: duration, index: index})).sort((a,b) => a.duration-b.duration))
        // console.log(data.sort((a,b) => a.duration-b.duration).map(({title, duration}, index) => ({title: title, duration: duration, index: index+1})))
        console.log(data.sort((a, b) => a.duration - b.duration).map(({ title, duration }, index) => (`${index + 1}: ${title}, ${duration}`)).join("\n"))
    } else {
        console.log(data.sort((a, b) => b.duration - a.duration).map(({ title, duration }, index) => (`${index + 1}: ${title}, ${duration}`)).join("\n"))
    }
}
